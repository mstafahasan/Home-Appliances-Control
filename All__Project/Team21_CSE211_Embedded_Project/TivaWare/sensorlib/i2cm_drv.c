                  ...
                  uint32_t ui32Status)
{
    ...
    if(pfnCallback)
    {
        ...
        // driver status.
        //
        if(ui32Status == I2C_MASTER_ERR_NONE)
        {
            ui32Status = I2CM_STATUS_SUCCESS;
        }
        else if(ui32Status & I2C_MASTER_ERR_ARB_LOST)
        {
            ui32Status = I2CM_STATUS_ARB_LOST;
        }
        else if(ui32Status & I2C_MASTER_ERR_ADDR_ACK)
        {
            ui32Status = I2CM_STATUS_ADDR_NACK;
        }
        else if(ui32Status & I2C_MASTER_ERR_DATA_ACK)
        {
...
I2CMIntHandler(tI2CMInstance *psInst)
{
    ...
    ui32Status = MAP_I2CMasterErr(psInst->ui32Base);
    ...
    //
    if((ui32Status != I2C_MASTER_ERR_NONE) && (psInst->ui8State != STATE_IDLE))
    {
        //
    ...
    while(1)
    {
        ...
        switch(psInst->ui8State)
        {
            ...
            case STATE_CALLBACK:
            {
                ...
                // the error handling stop condition above will cause the next
