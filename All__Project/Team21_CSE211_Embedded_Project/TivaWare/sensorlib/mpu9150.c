...
#define MPU9150_STATE_INIT_USER_CTRL                                          \
                                7           // init user control and pwr mgmt 1
#define MPU9150_STATE_INIT_SAMPLE_RATE_CFG                                    \
                                8           // init the sensors and filters
#define MPU9150_STATE_INIT_I2C_SLAVE_DLY                                      \
                                9          // set the ak8975 polling delay
#define MPU9150_STATE_INIT_I2C_SLAVE_0                                        \
                                10          // config ak8975 automatic read
#define MPU9150_STATE_RD_DATA   11          // Waiting for data read
...
MPU9150Callback(void *pvCallbackData, uint_fast8_t ui8Status)
{
    ...
    switch(psInst->ui8State)
    {
        ...
        case MPU9150_STATE_INIT_RESET_WAIT:
        {
            ...
            //
            if(psInst->pui8Data[0] & MPU9150_PWR_MGMT_1_DEVICE_RESET)
            {
                //
                // Device still in reset so beging polling this register.
                //
                psInst->uCommand.pui8Buffer[0] = MPU9150_O_PWR_MGMT_1;
                I2CMRead(psInst->psI2CInst, psInst->ui8Addr,
                         psInst->uCommand.pui8Buffer, 1, psInst->pui8Data, 1,
            ...
            {
                //
                // Device is out of reset, start init of user control register.
                //
                psInst->uCommand.pui8Buffer[0] = MPU9150_O_USER_CTRL;
                psInst->uCommand.pui8Buffer[1] = MPU9150_USER_CTRL_I2C_MST_EN;
                psInst->uCommand.pui8Buffer[2] = MPU9150_PWR_MGMT_1_CLKSEL_XG;
                I2CMWrite(psInst->psI2CInst, psInst->ui8Addr,
                          psInst->uCommand.pui8Buffer, 3, MPU9150Callback,
                          psInst);
        ...
        //
        // Reset is complete, device is ready.
        //
            ...
            void *pvCallbackData)
{
    ...
    //
    psInst->pui8Data[0] = MPU9150_O_PWR_MGMT_1;
    psInst->pui8Data[1] = MPU9150_PWR_MGMT_1_DEVICE_RESET;
