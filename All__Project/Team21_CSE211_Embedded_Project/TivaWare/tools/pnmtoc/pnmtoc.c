...
//*****************************************************************************
void
GetNumColors(uint8_t *pui8Data, uint32_t ui32Width,
             uint32_t ui32Height, uint32_t ui32Mono)
{
...
//*****************************************************************************
uint32_t
Encode1BPP(uint8_t *pui8Data, uint32_t ui32Width,
           uint32_t ui32Height, uint32_t ui32Mono)
{
...
//*****************************************************************************
uint32_t
Encode4BPP(uint8_t *pui8Data, uint32_t ui32Width,
           uint32_t ui32Height, uint32_t ui32Mono)
{
...
//*****************************************************************************
uint32_t
Encode8BPP(uint8_t *pui8Data, uint32_t ui32Width,
           uint32_t ui32Height, uint32_t ui32Mono)
{
...
{
    uint32_t ui32EncodedLength, ui32Idx, ui32Size, ui32Match, ui32MatchLen, ui32Count;
    ...
    for(ui32Count = 0; ui32Count < ui32Length; ui32Count++)
    {
        ...
        //
        for(ui32Idx = 0, ui32MatchLen = 0; ui32Idx < sizeof(pui8Dictionary); ui32Idx++)
        {
            //
            ...
            if(pui8Dictionary[ui32Idx] == pui8Data[ui32Count])
            {
                ...
                //
                for(ui32Size = 1; (ui32Idx + ui32Size) < sizeof(pui8Dictionary);
                    ui32Size++)
                {
...
//*****************************************************************************
void
OutputData(uint8_t *pui8Data, uint32_t ui32Width,
           uint32_t ui32Height, uint32_t ui32Length)
{
    ...
    //
    printf("const uint8_t g_pucImage[] =\n");
    printf("{\n");
    ...
    //
    printf("    %ld, %ld,\n", ui32Width & 255, ui32Width / 256);
    printf("    %ld, %ld,\n", ui32Height & 255, ui32Height / 256);
    printf("\n");
    ...
    if(g_ui32NumColors > 2)
    {
        ...
        for(ui32Idx = 0; ui32Idx < g_ui32NumColors; ui32Idx++)
        {
            printf("    0x%02lx, 0x%02lx, 0x%02lx,\n",
                   g_pui32Colors[ui32Idx] & 255, (g_pui32Colors[ui32Idx] >> 8) & 255,
...
{
    uint32_t ui32Length, ui32Width, ui32Height, ui32Max, ui32Count, ui32Mono, ui32Bitmap;
    uint8_t *pui8Data;
    ...
    //
    ui32Mono = (pui8Data[1] == '5') ? 1 : 0;

    //
    // Are we dealing with a (1bpp) bitmap?
    //
    ui32Bitmap = (pui8Data[1] == '4') ? 1 : 0;
    ...
    for(ui32Count = 0, ui32Max = 2; ui32Count < 3; )
    {
        ...
        if(ui32Count == 0)
        {
            if(sscanf(pui8Data + ui32Max, "%lu", &ui32Width) != 1)
            {
        ...
        {
            if(sscanf(pui8Data + ui32Max, "%lu", &ui32Height) != 1)
            {
        ...
        else
        {
            ...
            // never get here.
            //
            if(sscanf(pui8Data + ui32Max, "%lu", &ui32Count) != 1)
            {
    ...
    if(!ui32Bitmap)
    {
        ...
        //
        GetNumColors(pui8Data + ui32Max, ui32Width, ui32Height, ui32Mono);
        ...
        if(g_ui32NumColors <= 2)
        {
            ...
            // per pixel.
            //
            ui32Length = Encode1BPP(pui8Data + ui32Max, ui32Width, ui32Height, ui32Mono);
        }
        else if(g_ui32NumColors <= 16)
        {
            //
            ...
            // 4 bits per pixel.
            //
            ui32Length = Encode4BPP(pui8Data + ui32Max, ui32Width, ui32Height, ui32Mono);
        }
        else if(g_ui32NumColors <= 256)
        {
            ...
            // 8 bits per pixel.
            //
            ui32Length = Encode8BPP(pui8Data + ui32Max, ui32Width, ui32Height, ui32Mono);
        }
        else
        {
            //
    ...
    else
    {
        ...
        for(ui32Count = 0; ui32Count < ui32Length; ui32Count++)
        {
            *(pui8Data + ui32Max + ui32Count) = ~(*(pui8Data + ui32Max + ui32Count));
        }
    }
