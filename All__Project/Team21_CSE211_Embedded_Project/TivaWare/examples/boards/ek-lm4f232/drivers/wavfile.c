...
WavOpen(const char *pcFileName, tWavFile *psWavData)
{
    ...
    //
    if(f_read(&psWavData->i16File, pucBuffer, 12, &ui16Count) != FR_OK)
    {
        return(-1);
    }
    ...
    //
    if(f_read(&psWavData->i16File, pucBuffer, 8, &ui16Count) != FR_OK)
    {
        return(-1);
    }
    ...
    // Read the next chunk header.
    //
    if(f_read(&psWavData->i16File, pucBuffer, ui32ChunkSize, &ui16Count) != FR_OK)
    {
        return(-1);
    }
...
WavClose(tWavFile *psWavData)
{
    if(psWavData->ui32Flags & WAV_FLAG_FILEOPEN)
    {
        ...
        //
        // Mark file as no int32_ter open.
        //
        psWavData->ui32Flags &= ~WAV_FLAG_FILEOPEN;
    }
}
...
WavRead(tWavFile *psWavData, unsigned char *pucBuffer, uint32_t ui32Size)
{
    ...
    //
    if(f_read(&psWavData->i16File, pucBuffer, ui32Size, &ui16Count) != FR_OK)
    {
        return(0);
    }
