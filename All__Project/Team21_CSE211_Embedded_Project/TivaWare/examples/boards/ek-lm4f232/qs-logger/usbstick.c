    ...
    if(g_iState == eSTATE_DEVICE_READY)
    {
        ...
        if(!pcFilename8 || !pcFilename8[0])
        {
            ...
            iFResult = f_write(&g_sFileObject, g_pcCSVHeaderLine,
                              sizeof(g_pcCSVHeaderLine), &ui16BytesWritten);
            if(iFResult != FR_OK)
            {
                g_ui32Flags &= ~FLAGS_FILE_OPENED;
                return(0);
...
USBStickWriteRecord(tLogRecord *psRecord)
{
    ...
    //
    iFResult = f_write(&g_sFileObject, pcBuf, ui32BufIdx, &ui16BytesWritten);

    //
    // Check for errors
    //
    if((iFResult != FR_OK) || (ui16BytesWritten != ui32BufIdx))
    {
        //
        // Some error occurred
        //
